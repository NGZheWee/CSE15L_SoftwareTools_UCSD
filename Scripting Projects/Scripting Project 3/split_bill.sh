#!/bin/bash
# Complete the first line of the script with the correct location for
# bash
#
...
function create_billings {
	billings=()
	# Loop through NAMES array, using some index variable
	for (( index2=0; index2<${#NAMES[*]}; index2++ ))
	do
		# zero initialize the current index in the billings array.
		billings[$index2]=0;
	done
}
...
function assign_dishes {
	# Loop through dishes array, using some index variable
	...
	for (( index3=0; index3<${#dishes[*]}; index3++ ))
	do
		# Access the current dish.
		dish=${dishes[$index3]}
		
		# Access the price of the current dish.
		price=${prices[$index3]}

		echo $RECEIPT_MSG

		# Print the current item number, followed by the name of the dish and its price.
		echo -e "\t[`expr $index3 + 1`] $dish\t$price"

		# call the print_names function.
		print_names
		
		# Print the ASSIGN_MESSAGE prompt
		echo -e $ASSIGN_MESSAGE

		# Store the selected name in the assigned_person variable.
...
#   get_tax
function get_tax {
	# show the TAX_MESSAGE to the user.
	echo -n -e $TAX_MESSAGE

	# store the value entered by the user in a variable called "tax"
	read tax
}
...
#   get_tip
function get_tip {
	# show the TIP_MESSAGE to the user.
	echo -n -e $TIP_MESSAGE

	# store the value entered by the user in a variable called "tip"
	read tip
}
...
function calculate {
	...
	# Loop through billings array, using some index variable
	for (( index4=0; index4<${#billings[*]}; index4++ ))
	do
		# Access the current element of the billings array.
		bill=${billings[$index4]}

		# Add the tax
		# Note we add (denom - 1) to the numerator as a way of rounding up
		tax_amount=$[($bill * $tax + 50) / 100]

		# Add the tax_amount to the bill.
		bill=$(($tax_amount+$bill))

		# Add the tip
		# Note we add (denom - 1) to the numerator as a way of rounding up
		tip_amount=$[($bill * $tip + 50) / 100]

		# Add the tip_amount to the bill
		bill=$(($tip_amount+$bill))

		# Calculate the total amount.
		# Convert the current bill from cents to dollars using convert_cents_to_dollars
		total_dollars=$(convert_cents_to_dollars $bill)

		# Print the correct name.
		echo "${NAMES[$index4]} owes $total_dollars"
	done
}
